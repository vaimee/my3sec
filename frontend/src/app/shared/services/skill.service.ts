...
import { Observable, catchError, concatMap, forkJoin, map, mergeMap, switchMap, toArray } from 'rxjs';
...
export class SkillService {
  ...
  public getProfileSkills(profileId: number): Observable<ProfileSkill[]> {
    return this.skillWallet.getSkillCount(profileId).pipe(
      ...
      concatMap(data => data),
      mergeMap(([skillId, progress]) => {
        return this.getSkill(skillId.toNumber()).pipe(
          map(skillData => ({ ...skillData, progress: progress.toNumber() }))
        );
      }),
      toArray()
    );
